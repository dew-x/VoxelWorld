...
GLuint TextureManager::loadTexture(std::string filePath) {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		*/
	unsigned char* image = SOIL_load_image(filePath.c_str(), &_texture.width, &_texture.height, 0, SOIL_LOAD_RGB);
	...
	if (image == NULL) {
		...
		*/
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, _texture.width, _texture.height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
